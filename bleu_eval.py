...
# rnn+attention 的bleu评分

CHECKPOINT_PATH = "checkpoint/attention/attention-1720"
HIDDEN_SIZE = 1024  # LSTM的隐藏层规模
NUM_LAYERS = 2   # 深层循环神经网络中LSTM结构的层数
SRC_VOCAB_SIZE = 3014  # 源语言词汇表大小
TRG_VOCAB_SIZE = 3416  # 目标语言词汇表大小
BATCH_SIZE = 1  # 测试数据的batch的大小
SHARE_EMB_AND_SOFTMAX = True  # 在softmax层和词向量之间共享参数

# 词汇表文件
SRC_VOCAB = "data/cn_vocab.txt"
TRG_VOCAB = "data/en_vocab.txt"
...
class NMTModelA(object):
    ...
    def inference(self, src_input, src_size):
        # 将输入单词转为词向量
        ...
        with tf.variable_scope("decoder/rnn/attention_wrapper"):
            ...
            # 步数的一个整数step
            init_loop_var = (
                attention_cell.zero_state(batch_size=batch_size, dtype=tf.float32),
                init_array, 0)
...
EN_TEST_IDS = "data/cn_test_ids.txt"
CN_TEST_IDS = "data/en_test_ids.txt"
...
def bleu():
    ...
    if not os.path.exists('results'):
        os.mkdir('results')
    fout = open("results/result.txt", "w", encoding="utf8")
    with tf.variable_scope("nmt_model", reuse=None):
        model = NMTModelA()
    with open(TRG_VOCAB, "r", encoding="utf8") as f_vocab:
        trg_vocab = [w.strip() for w in f_vocab.readlines()]
    with open("data/en_test.txt", "r", encoding="utf8") as fp:
        en_test_set = fp.readlines()
    with open("data/cn_test.txt", "r", encoding="utf8") as fp:
        cn_test_set = fp.readlines()
    # 定义输入数据
    data = MakeSrcTrgDataset(EN_TEST_IDS, CN_TEST_IDS, BATCH_SIZE)
    iterator = data.make_initializable_iterator()
    (src, src_size), _ = iterator.get_next()
    output_op = model.inference(src, src_size)
    ...
    with tf.Session() as sess:
        ...
        while True:
            try:
                ...
                fout.write("- source: " + cn_test_set[count].strip() + "\n")
                fout.write("- target: " + en_test_set[count].strip() + "\n")
                fout.write("- output: " + output_text + "\n\n")
                fout.flush()
                # bleu score
                ref = cn_test_set[count].strip().split()
