...
SRC_TRAIN_DATA = "data/cn_train_ids.txt"  # 源语言输入文件
TRG_TRAIN_DATA = "data/en_train_ids.txt"  # 目标语言输入文件
...
class NMTModel(object):
    ...
    def __init__(self):
        ...
        self.dec_cell = tf.nn.rnn_cell.MultiRNNCell(
            [tf.nn.rnn_cell.LSTMCell(HIDDEN_SIZE) for _ in range(NUM_LAYERS)])
        # 为源语言和目标语言分别定义词向量
        self.src_embedding = tf.get_variable('src_emb', [SRC_VOCAB_SIZE, HIDDEN_SIZE])  # （3000 * 1000）
        self.trg_embedding = tf.get_variable('trg_emb', [TRG_VOCAB_SIZE, HIDDEN_SIZE])  # （3000 * 1000）
        # 定义softmax层的变量
    ...
    def forward(self, src_input, src_size, trg_input, trg_label, trg_size):
        ...
        # 将输入和输出单词转为词向量
        # src_input的维度是(batch_size, num_steps),词嵌入层的输出为(batch_size, num_steps, hidden_size)
        ...
        # (batch_size, step_size, hidden_size)
...
class NMTModelA(object):
    ...
    def __init__(self):
        ...
        # 定义softmax层的变量   softmax层和词嵌入层共享参数
        if SHARE_EMB_AND_SOFTMAX:
            self.softmax_weight = tf.transpose(self.trg_embedding)
        else:
            self.softmax_weight = tf.get_variable('weight', [HIDDEN_SIZE, TRG_VOCAB_SIZE])
